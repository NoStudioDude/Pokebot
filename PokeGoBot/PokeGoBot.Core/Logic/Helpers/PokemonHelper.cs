...
namespace PokeGoBot.Core.Logic.Helpers
{
    public interface IPokemonHelper
    {
        ...
        bool ShouldTranferPokemon(PokemonData pokemon, int minPercentageIvToDiscart, double minCpToKeep, bool ivOverCp);
    }

    public class PokemonHelper : IPokemonHelper
    {
        public bool ShouldTranferPokemon(PokemonData pokemon, int minPercentageIvToDiscart, double minCpToKeep, bool ivOverCp)
        {
            var iv = pokemon.IndividualAttack + pokemon.IndividualDefense + pokemon.IndividualStamina;
            var ivPercentage = (double)iv / 45;
            return ivOverCp ? (ivPercentage < minPercentageIvToDiscart) : pokemon.Cp <= minCpToKeep;
        }
